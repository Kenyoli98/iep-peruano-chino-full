  ...
  try {
    ...
    if (error.code === 'P2002') {
      ...
      if (
        ...
        error.meta?.target?.includes('Curso_nombre_key')
      ) {
        return res
          .status(400)
...
async function crearCursosMasivo(req, res) {
  ...
  try {
    ...
    res
      .status(201)
      .json({ mensaje: 'Cursos creados.', cantidad: nuevosCursos.count });
  } catch (error) {
    console.error(error);
  ...
  try {
    ...
    if (filas.length <= 1) {
      return res
        ...
        .json({
          error:
            'El archivo CSV debe contener al menos una fila de datos además de la cabecera.'
        });
    }
    ...
    if (!headers.includes('nombre')) {
      return res
        .status(400)
        .json({ error: 'El archivo CSV debe contener una columna "nombre".' });
    }
    ...
    if (cursos.length === 0) {
      return res
        ...
        .json({
          error:
            'No se encontraron cursos válidos en el archivo CSV. Asegúrate de que la columna "nombre" contenga datos.'
        });
    }
