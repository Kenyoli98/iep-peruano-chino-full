  ...
  try {
    ...
    for (const [nombre, gruposCursos] of Object.entries(cursosAgrupados)) {
      if (gruposCursos.length > 1) {
        console.log(`Encontrados ${gruposCursos.length} cursos con nombre "${nombre}"`);
        ...
        for (const curso of cursosAEliminar) {
          console.log(`Procesando curso duplicado: ID ${curso.id}, Nombre: "${curso.nombre}"`);
          ...
          if (asignaciones.length > 0) {
            console.log(`  - Moviendo ${asignaciones.length} asignaciones al curso principal (ID: ${cursoAMantener.id})`);
