...
async function normalizarNiveles() {
  try {
    ...
    console.log('Niveles encontrados:', nivelesUnicos.map(n => `"${n.nivel}"`));
    ...
    if (inicialResult.count > 0) {
      actualizaciones.push(`${inicialResult.count} secciones de nivel inicial normalizadas a INICIAL`);
    }
    ...
    if (primariaResult.count > 0) {
      actualizaciones.push(`${primariaResult.count} secciones de nivel primaria normalizadas a PRIMARIA`);
    }
    ...
    if (secundariaResult.count > 0) {
      actualizaciones.push(`${secundariaResult.count} secciones de nivel secundaria normalizadas a SECUNDARIA`);
    }
    ...
    console.log('\n🔍 Niveles después de la normalización:', nivelesFinales.map(n => `"${n.nivel}"`));
