...
async function probarCreacionDuplicados() {
  ...
  try {
    ...
    const cursosResponse = await makeRequest('GET', '/cursos?nombre=PRUEBA_DUPLICADO', null, token);
    if (cursosResponse.data.cursos && cursosResponse.data.cursos.length > 0) {
      const cursoId = cursosResponse.data.cursos[0].id;
      const deleteResponse = await makeRequest('DELETE', `/cursos/${cursoId}`, null, token);
