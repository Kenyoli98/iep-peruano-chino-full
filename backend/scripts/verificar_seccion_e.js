  ...
  try {
    ...
    if (asignaciones.length > 0) {
      ...
      asignaciones.forEach((asignacion, index) => {
        console.log(`   ${index + 1}. Profesor: ${asignacion.profesor.nombre} ${asignacion.profesor.apellido}`);
      ...
      console.log('   2. Eliminar todas las asignaciones de profesores a esta sección');
      ...
      if (matriculas.length > 0) {
        ...
        matriculas.forEach((matricula, index) => {
          console.log(`   ${index + 1}. ${matricula.estudiante.nombre} ${matricula.estudiante.apellido}`);
        });
