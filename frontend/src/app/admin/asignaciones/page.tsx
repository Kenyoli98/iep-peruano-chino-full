...
import {
  obtenerSecciones,
  type Seccion,
  type SeccionesResponse
} from '@/services/seccionesService';
import AsignacionesModal, {
  HorarioProfesor
} from '@/components/asignaciones/modals/ModalAsignaciones';
...
export default function AsignacionesAdminPage() {
  ...
  const fetchDatosIniciales = async () => {
    try {
      ...
      const [profesoresData, cursosData, seccionesResponse] = await Promise.all(
        [obtenerProfesores(), obtenerCursos(), obtenerSecciones()]
      );
      ...
      setSecciones(
        Array.isArray(seccionesData) ? seccionesData : seccionesData.secciones
      );

      console.log('Datos procesados:', {
        profesoresCount: profesoresData?.length || 0,
        cursosCount: cursosData?.length || 0,
        seccionesCount: Array.isArray(seccionesData)
          ? seccionesData.length
          : seccionesData.secciones?.length || 0
      });
    } catch (error: any) {
  ...
  const handleSeleccionarProfesor = async (profesor: Profesor) => {
    try {
      ...
      setError(
        error.message || 'Error al cargar las asignaciones del profesor'
      );
    } finally {
      setLoading(false);
    }
  ...
  const profesoresFiltrados = profesores.filter(profesor =>
    `${profesor.nombre} ${profesor.apellido}`
      .toLowerCase()
      .includes(busquedaProfesor.toLowerCase())
  );
        ...
        >
          <div className='p-8'>
            ...
            <div className='print-title' style={{ display: 'none' }}>
              ...
              <p>Año Académico {new Date().getFullYear()}</p>
            </div>
