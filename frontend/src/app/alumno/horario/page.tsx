    ...
    <div className="space-y-6">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {horasClase.map((hora, index) => (
                <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  {diasSemana.map((dia) => (
                    <td key={`${dia}-${index}`} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <div className="flex items-start">
          ...
          <div className="ml-3">
            <h3 className="text-sm font-medium text-blue-900">Información del Horario</h3>
            <p className="mt-1 text-sm text-blue-700">
              Tu horario de clases se actualizará automáticamente cuando el administrador asigne los cursos y horarios.
              Si tienes dudas sobre tu horario, contacta a la administración del colegio.
            </p>
          </div>
        </div>
      </div>
