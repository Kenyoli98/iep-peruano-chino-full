...
const StatCard = ({ number, label, delay = 0, isVisible }: { number: string; label: string; delay?: number; isVisible: boolean }) => (
  <div 
...
export default function HeroSection() {
  ...
  return (
    <section 
      className="relative flex flex-col items-center justify-center text-center py-16 md:py-28 px-4 min-h-[80vh] overflow-hidden"
    >
      ...
      >
        ...
        <div className="absolute bottom-20 right-10 w-40 h-40 bg-white rounded-full blur-xl animate-pulse" style={{ animationDelay: '1s' }}></div>
        <div className="absolute top-1/2 left-1/4 w-24 h-24 bg-white rounded-full blur-xl animate-pulse" style={{ animationDelay: '2s' }}></div>
      </div>
      ...
      <div className={`relative z-20 max-w-4xl mx-auto transition-all duration-1000 ${
        ...
        <h1 className={`text-4xl md:text-6xl lg:text-7xl font-bold mb-6 text-white drop-shadow-2xl leading-tight transition-all duration-1000 delay-200 ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        }`}>
          I.E.P Peruano Chino
        </h1>
        <h2 className={`text-xl md:text-2xl lg:text-3xl font-medium text-white/90 drop-shadow-lg mb-8 leading-relaxed max-w-3xl mx-auto transition-all duration-1000 delay-400 ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        }`}>
          Formando líderes del mañana con excelencia académica y valores sólidos
        </h2>
        ...
        {/* Estadísticas */}
        <div className={`grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto transition-all duration-1000 delay-800 ${
          isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
        }`}>
          <StatCard number="25+" label="Años de Experiencia" delay={1000} isVisible={isVisible} />
          <StatCard number="500+" label="Estudiantes Graduados" delay={1200} isVisible={isVisible} />
          <StatCard number="98%" label="Satisfacción Familiar" delay={1400} isVisible={isVisible} />
        </div>
      </div>
