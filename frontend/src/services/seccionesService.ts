...
export const obtenerSecciones = async (params: SeccionesFiltros = {}): Promise<Seccion[]> => {
  try {
    ...
    if (res.data?.secciones && Array.isArray(res.data.secciones)) {
      ...
      console.log('Número de secciones obtenidas (array directo):', res.data.length);
      secciones = res.data;
    } else {
    ...
    console.error('Error detallado al obtener secciones:', error.response || error);
    if (error.response?.status === 401) {
      throw new Error('Tu sesión ha expirado. Por favor, inicia sesión nuevamente.');
    }
    throw new Error('Error al obtener la lista de secciones: ' + (error.response?.data?.message || error.message));
  }
};

export const obtenerSeccionesPaginadas = async (params: SeccionesFiltros = {}): Promise<SeccionesResponse> => {
  try {
    const token = localStorage.getItem('token');
    const res = await api.get<SeccionesResponse>('/secciones', {
      params,
...
export const crearSeccion = async (seccion: { nombre: string; nivel: string; grado: string }) => {
  try {
    const token = localStorage.getItem('token');
    ...
    throw new Error('Error al crear la sección: ' + (error.response?.data?.error || error.response?.data?.message || error.message));
  }
};

export const actualizarSeccion = async (id: number, seccion: { nombre: string; nivel: string; grado: string }) => {
  try {
    const token = localStorage.getItem('token');
    // Enviar todos los datos como string según requerimiento
    const seccionData = {
    ...
    throw new Error('Error al actualizar la sección: ' + (error.response?.data?.error || error.response?.data?.message || error.message));
  }
};
